<template>
    ...
    <div class="form-container" v-if="isLogin">
      <form @submit.prevent="handleSubmit">
        ...
        <!-- 3.시급, 월급 항목 선택 -->
        <div class="form-group">
          <input type="radio" id="pay_rule1" name="pay_rule" value="시급" required v-model="pay_rule"  checked>
          <input type="radio" id="pay_rule2" name="pay_rule" value="월급" required v-model="pay_rule"  >
          <div class="tab-group">
            <label for="pay_rule1">시급</label>
            <label for="pay_rule2">월급</label>
          </div>
          <!-- 4.금액 입력 -->
          <input type="number" id="pay" required placeholder="시급 또는 월급을 입력해주세요." v-model="pay" >
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" id="tel"  required placeholder="예) 010-1234-5678" v-model="tel">
        </div>
...
<style lang="scss" scoped>
  ...
  .form-group:has(label[for=photo]) {
    padding-bottom: 25px;
    border-bottom: 5px solid #ccc;  // 하단 경계선
  }
</style>
